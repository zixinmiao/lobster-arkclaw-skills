...
- “建议触达时机”可以输出为抽象类型，如 `next_day`、`weekend_or_holiday`、`event_triggered`，但**不得擅自生成具体日期、具体活动名或具体折扣力度**，除非上游已明确提供。
...
说明：
- 若没有真实活动/折扣/到货信息，`recommended_touch_time` 优先输出相对时机或空值，不要伪造具体日期。
...
- `recommended_action`
- `priority`
- `status` (`pending` / `waiting_trigger` / `cancelled`)
- `confidence`
