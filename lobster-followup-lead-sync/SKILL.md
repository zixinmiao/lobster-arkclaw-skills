...
## 共享配置
执行前先读取 `../configs/feishu-bases.json`：
- `bases.tryon`：试衣主表绑定
- `bases.followup`：线索回访表绑定
- 不要在 skill 里重新硬编码 token / table id

当前线索回访表绑定：
- `baseToken`: `MRUXbR0xnaUjnTsdMVrcFjiTnYb`
- `tableId`: `tbll0eeYftM9CLSc`
- `viewId`: `vewleQBhtM`
...
### followup_lead
必须对齐当前真实字段（见 `../configs/feishu-bases.json -> bases.followup.fields`），推荐输出：
- `线索ID`
- `来源试衣记录ID`
- `日期`
- `导购`
...
- `提醒时间` 字段是 `datetime`，推荐写 `YYYY-MM-DD HH:mm:ss`。
