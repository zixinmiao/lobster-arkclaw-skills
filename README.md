...
- 新增 `lobster-followup-lead-sync`：基于试衣反馈判断是否生成回访线索，并同步更新试衣表与线索回访表，支持输出回访原因、建议触达时机、触达触发条件和建议回访内容。
- 新增 `lobster-followup-reminder-dispatch`：用于配合统一定时任务扫描线索回访表，派发到期提醒，避免为每条线索单独手工配置定时任务。
