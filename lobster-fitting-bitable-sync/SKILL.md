...
- 当前默认不依赖预置 `config.json`。优先通过“自动发现现有表 + 人工确认”获取 binding。
...
## 自动发现机制
如果当前没有可用 `bitable_binding`，先发现，再确认，再写入：

### 1. 搜索候选表
优先搜索以下名称相近的 Base / Table：
- `导购小龙虾试衣台账`
- `试衣商品记录`
- `线索回访表`
- `会员画像`
...
### 2. 校验字段匹配度
命中候选后，不按名字直接认定，必须读取字段结构并和预期字段比对。
...
若发现 1 个或多个高匹配候选：
- 输出候选表名
- 输出 `base_token / table_id / view_id`（若可取到）
- 输出字段匹配度摘要
- 明确标记 `needs_human_confirmation = true`

### 4. 确认后再持久化 binding
- 只有在用户确认后，才把该 binding 视为正式可写目标
- 确认前禁止自动向候选表写正式业务记录
...
## 执行规则
- 缺 binding 时，不要伪造一个固定表配置；应先进入自动发现流程。
- 若自动发现得到候选但尚未确认，`sync_status` 应标记为 `needs_confirmation` 或 `no_binding`。
