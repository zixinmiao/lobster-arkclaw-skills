...
- 会员识别结果
- 上下文中的门店、导购、客户、消息来源
- `sender_profile` / `guide_profile`（可选；用于继承导购姓名、门店等稳定资料）
- 图片/语音素材索引
- 最近一次 open session 或候选 session 信息
- 飞书拆分消息列表（可选）
- `context_policy`（可选，`inherit_if_safe` / `strict_isolation`）
- `force_ignore_history`（可选，`true` 表示本次判断尽量仅基于当前 bundle）
- `force_new_session`（可选，`true` 表示本次不得 append 到历史 session）
...
- **若当前 bundle 中出现新的吊牌 OCR 商品结果，不能仅因 session 匹配就直接沿用旧商品字段覆盖本次商品主值**
- **当 `context_policy = strict_isolation` 时，append 必须满足更高门槛：除“同一天 + 时间接近”外，还必须存在明确延续语义，且至少一个强锚点（商品/会员/门店/客户指代）高匹配；否则不得 append**
...
- **命中“重新开始 / 清掉上一条 / 不要接着刚才 / 忽略前文 / 这是一条新的”这类明确重开语义**
...
- 商品识别结果不稳定或冲突
- 无法判断是补充上一条，还是新的一条
- **历史上下文虽然存在，但当前输入证据不足以安全续写；在 `strict_isolation` 模式下，优先 `pending_confirm`，不要勉强 append**
...
- **当 `force_ignore_history = true` 或 `context_policy = strict_isolation` 时，飞书拆分消息的归并与 session 判断应尽量只依据当前时间窗口内的 `messages`；窗口外历史消息只可作为弱参考，不得成为主导归属依据**
...
- **当当前输入包含新的吊牌 OCR 结果时，当前 `fitting_record` 的商品主字段（如 `product_code` / `product_name` / `sku`）应优先采用“本次 bundle 的 OCR 结果 + 本次文本补充”，不得默认被历史上下文商品覆盖**
- **若 OCR 结果与历史上下文商品冲突，应在 `merge_notes` 中明确记录冲突来源，并优先保留本次输入商品作为当前候选主值**
...
- **不得把历史上下文中的旧商品主值，无依据地覆盖本次吊牌 OCR 已识别出的商品主值**
- **不得仅因为上游传入了 `recent_sessions` 或历史消息，就默认认定当前输入与历史属于同一单；skill 必须先证明“安全可续写”，否则应新开或待确认**


### F. 导购资料继承规则
- 若当前消息未再次显式提供 `guide_name` / `store_name`，但 `sender_profile` 中已有稳定值，应直接继承
- `source_sender_id` 应优先直接取自入站消息 metadata（如飞书 sender open_id），而不是由模型从正文中推断
- 不得因为单条消息缺少姓名/门店，就忽略同一 sender 在历史中已经明确给过的信息
- 只有在 `sender_profile` 不存在、或当前输入与历史资料冲突时，才允许将其标记到 `missing_fields`
- `source_sender_id` 与 `source_chat_id` 应作为 sender profile 继承与定位的主锚点
- 若来源渠道为飞书且入站 metadata 已提供 open_id，则 `source_sender_id` 不得标记为缺失
